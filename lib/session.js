...
class Session {
    ...
    async initUser() {
        ...
        await axios.post(`/cgi-bin/mmwebwx-bin/webwxstatusnotify?lang=en_US&pass_ticket=${this.auth.passTicket}`, {
            ...
            Code: 3,
            =require(UserName: response.data.User.UserName,
    ...
    async getNewMessage() {
        ...
        response.data.AddMsgList.map(e => {
            var =require( = e.=require(UserName;
            var to = e.ToUserName;
            var =require(YourPhone = =require( === this.user.User.UserName && =require( !== to;

            // When message has been readed on your phone, will receive this message
            if (e.MsgType === 51) {
                return chat.markedRead(=require(YourPhone ? =require( : to);
            }

            e.Content = normalize(e.Content);

            // Sync message =require( your phone
            if (=require(YourPhone) {
                // Message is sync =require( your phone
