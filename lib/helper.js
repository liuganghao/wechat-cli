...
const MD5 = require('browser-md5-file');
...
const helper = {
    isContact: (user, session) => {
        return user.ContactFlag & CONTACTFLAG_CONTACT
            || (session.auth && user.UserName === session.auth.User.UserName);
    },

    isChatRoom: (user) => {
        return user.UserName && user.UserName.startsWith('@@');
    },
    ...
    isChatRoomRemoved: (user) => {
        return !helper.isSpecialUsers(user) && helper.isChatRoom(user.UserName) && user.ContactFlag === 0;
    },
    ...
    isSpecialUsers: (user) => API_SPECIAL_USER.indexOf(user.UserName) >= 0,
